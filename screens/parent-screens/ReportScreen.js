...
import { View, Text, StyleSheet, Dimensions, TextInput,ToastAndroid, FlatList, TouchableOpacity,YellowBox,Modal } from 'react-native';
...
import { Asset, Constants, FileSystem, Permissions,Videos} from 'react-native-unimodules';
...
const ReportScreen = () => {
  ...
  const [selectedEmails, setSelectedEmails]=useState([{id:"medhavi.srivastava16@gmail.com", isSelected:true}, {id:"imt_2018050@iiitm.ac.in",isSelected:true}]) 
 ...
 const pickFromGallery = async () => {
  ...
  if (granted) {
    ...
    if (!data.cancelled) {
      ...
      console.log("uri="+newFile.uri+" "+"name="+newFile.filename+" base64=="+itemData.base64)
    }
  } else {
...
//On Anonymous User Press Actions 
  const onSecretUserPress=()=>{
      ...
      {
        ToastAndroid.show("Send to vounteer to hide identity", ToastAndroid.SHORT);
      }
      else
      {
        ToastAndroid.show("Directly send complaint to the portal with your identity", ToastAndroid.SHORT);
      } 
  }
  ...
  const onForwardPress=()=>{
    ...
    if(!pressedStateSecretUser)
    {
      ToastAndroid.show("Forward to selected emergency contacts", ToastAndroid.SHORT);
    }
    else
    {
      ToastAndroid.show("Don't forward to selected emergency contacts", ToastAndroid.SHORT);
    } 
}
  ...
  return new Promise(async(res, rej) => {    
      ...
      if (!firebase.apps.length) {
            ...
            firebase.initializeApp({
              ...
              databaseURL: "https://cyberbullyingreactnativecli-default-rtdb.firebaseio.com",
...
const submitHandler= async ()=>{
  ...
  fetch('https://cyber-bullying1.herokuapp.com/api/mailing/report',{
    ...
    body:JSON.stringify({pressedStateSecretUser,
      pressedStateForward,
      selectedEmails,
      filesSelected,
      complaintText})
...
//props of secret user button
  var touchPropsSecretUser = {
    activeOpacity: 1,
    style: pressedStateSecretUser ? styles.configIconSelected : styles.configIcon,
    onPress: () => {onSecretUserPress()}               // <-- "onPress" is apparently required
  };
  ...
  return (
    ...
    <ScrollView contentContainerStyle={{justifyContent:'center', alignItems:'center'}}>
      ...
      {/* top row */}
      <View style={{flexDirection:'row', justifyContent:'space-between',marginTop: 40, width:Dimensions.get('window').width-10, height:50}}>
      ...
      <Text style={{fontWeight: '700', color: colors.white, fontSize: 30}}>File a report</Text>
      <TouchableOpacity style={{ elevation:10,marginStart: 10, alignSelf:'flex-end', width:50, height: 50, borderRadius: 30, backgroundColor:colors.primary, borderColor:colors.lightBlueGrey, borderWidth:1.5,paddingHorizontal: 10, paddingVertical: 8, justifyContent:'center'}}>
          <Icon
          name='phone'
          size={25}
          color={colors.white}
        ...
        <View style={{ flexDirection:'row',elevation:10, alignSelf:'center', width:Dimensions.get('window').width-40, height: 60, borderRadius: 35, backgroundColor:colors.blueGrey, borderColor:colors.blueGrey, borderWidth:1,  justifyContent:'center'}}>
          ...
          <TouchableOpacity onPress={pickImageFromGallery} style={styles.attachmentIcon}>
          <Icon
          name='images'
          size={25}
          color={colors.secondary}
          ...
          <TouchableOpacity onPress={pickImageFromCamera} style={styles.attachmentIcon}>
          <Icon
          name='camera-retro'
          size={25}
          color={colors.secondary}
          ...
          {/* Insert Video Icon */}
          <TouchableOpacity onPress={pickVideoFromGallery} style={styles.attachmentIcon}>
          <Icon
          name='file-video'
          size={25}
          color={colors.secondary}
          ...
          {filesSelected.length>0?<View style={styles.modalView}>
            <BrickList
            data={filesSelected}
       ...
       {/*Forward Area*/}
        <View style={{ flexDirection:'row',elevation:10, marginTop:20, alignSelf:'center', width:Dimensions.get('window').width-40, height: 60, borderRadius: 35, backgroundColor:colors.blueGrey, borderColor:colors.gray2, paddingEnd:10, borderWidth:1,  justifyContent:'center'}}>
        ...
        <TouchableOpacity style={styles.submit} onPress={uploadAll}>
            <Text style={{color:colors.black,  paddingHorizontal: 17, fontWeight:'700', fontSize:15, textAlign:'center'}}>SUBMIT</Text>
          </TouchableOpacity>
